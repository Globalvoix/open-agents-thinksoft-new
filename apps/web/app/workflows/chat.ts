...
const runAgentStep = async (
  ...
  try {
    ...
    if (finishReason === "other") {
      ...
      if ((stepUsage?.outputTokens ?? 0) === 0) {
        const errorWriter = writable.getWriter();
        try {
          await errorWriter.write({
            type: "error",
            errorText:
              "The AI model returned an empty response. This can happen when the conversation context is too large. Please try again or start a new chat.",
          });
        } finally {
